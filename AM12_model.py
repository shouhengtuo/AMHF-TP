...
class ETFC(nn.Module):
    def __init__(self, vocab_size, embedding_size, output_size, dropout, fan_epoch, num_heads, max_pool=5):
        ...
        self.full7 = nn.Linear(1280, 1152)
    ...
    def forward(self, train_data, valid_lens, hyGx_train, in_feat=False):


        # 这部分将二级结构不再拼接，直接在后面拼接bert特征
        ...
        hyg_data = hyGx_train # 这部分是bert特征 总共21维
        ...
        # AA_cnn_output:1152 normalized_output2:21    加起来1173维
        cnn_output0 = torch.cat([AA_cnn_output, normalized_output2], dim=-1)
