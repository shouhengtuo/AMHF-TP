...
class DataTrain:
        ...
        for epoch in range(1, epochs+1):
            ...
            with torch.no_grad():
                ...
                if val_aiming > best_aiming:
                    test_scores = evaluation.evaluate(np.array(test_pred), np.array(test_label))
                    print(f'-------------------------------aiming: {test_scores["aiming"]:.3f}')
                    print("测试集：")
                    print(f'aiming: {test_scores["aiming"]:.3f}')
                    print(f'coverage: {test_scores["coverage"]:.3f}')
                    print(f'accuracy: {test_scores["accuracy"]:.3f}')
                    print(f'absolute_true: {test_scores["absolute_true"]:.3f}')
                    print(f'absolute_false: {test_scores["absolute_false"]:.3f}')
                    best_aiming = val_aiming
                    best_test_pred = test_pred
                    torch.save(self.model.state_dict(), './result/AM13_1.pkl')
    ...
    def load_model_test(self, model, train_data):
        model.load_state_dict(torch.load('./result/AM13_1.pkl'))
        with torch.no_grad():
            model.eval()
            test_size = 0.1
            y_train = torch.Tensor(train_data['label'])
