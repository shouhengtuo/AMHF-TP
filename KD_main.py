...
def main(num, data):
    ...
    # test_score = evaluate(Student_model, dataset_test, device=DEVICE)
        ...
        for data_train, targets, train_length in dataset_train:
            ...
            with torch.no_grad():
                out1_t, out2_t, out3_t, out_t = Teacher_model(data_train.long(), train_length, in_feat=True)
                soft_label0 = list(nn.Sigmoid()(out_t.detach() / temp))
                soft_label.append(soft_label0)
            soft_label = torch.LongTensor(soft_label)
        # 学生模型预测
        # out1_s, out2_s, out3_s, out_s = Student_model(data_train.long(), train_length, in_feat=True)
        sequence_X, train_data = get_hyG("./dataset/target.txt")
        Student_model = Seq_HyGAN(sequence_X.shape[1], 64, 128, 128, 21, 0.3)
        Student_model.train()
        test_size = 0.1
        LEN = len(soft_label)
        train_label, test_label = train_test_split(soft_label, test_size=test_size, random_state=42)
        train_targets, test_targets = train_test_split(targets, test_size=test_size, random_state=42)
        ...
        hyG_fea = Student_model(train_data['hyG'], train_data['v_feat'], train_data['e_feat'], True, True)
        ...
        # 计算hard_loss和kd_loss
        stu_loss = criterionFDL(train_pred, train_targets)
        kd_loss = criterionCEL(train_pred / temp, soft_label.detach())
        # 将hard_loss和kd_loss加权求和
        loss = stu_loss + kd_loss * alpha
        # 反向传播, 优化权重
        Student_optimizer.zero_grad()
        ...
        print(f'[ Epoch {epoch + 1} ', end='')
        print("运行时间{}s".format(finish - start))
        print(f'loss={loss / len(dataset_train)} ]')
    test_score = evaluate(Student_model, dataset_test, device=DEVICE)
    "-------------------------------------------保存模型参数-----------------------------------------------"
    PATH = os.getcwd()
