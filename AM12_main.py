...
import numpy as np
import pandas as pd
from AM12_model import *
from torch.utils.data import DataLoader
from loss_functions import *
from AM23_train import *
...
def main(data):
    ...
    # 2、为数据集中增加bert模型特征: bert模型的输出在一个文件中
    # 从TXT文件中读取数据
    ...
    # 这是最初的拼接方式，直接拼接后，传入模型中，无法收敛，所以摈弃
    # # 记录拼接到第几行了
    # x_train = []
    # x_test = []
    # row_num = 0
    ...
    #     if row_num < 7873:  # 训练数据：7873条，序号对应：[0,7872]
    #         x = np.concatenate([x_train0[row_num], row])
    #         x_train.append(x.tolist())
    #     if (row_num >= 7873) & (row_num < 9843):  # 测试数据：[7873, 9842]
    #         y = np.concatenate([x_test0[test_index], row])
    ...
    dataset_train = list(zip(x_train, y_train, train_length, hyGx_train))
    dataset_test = list(zip(x_test, y_test, test_length, hyGx_test))
    ...
    torch.manual_seed(62325) # 0.652
    # torch.manual_seed(85325)
    ...
    with torch.no_grad():
        ...
        model.load_state_dict(torch.load('./result/AM12_1214.pkl'))
