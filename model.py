...
class ETFC(nn.Module):
    def __init__(self, vocab_size, embedding_size, output_size, dropout, fan_epoch, num_heads, max_pool=5):
        ...
        self.full7 = nn.Linear(2304, 1152)
    ...
    #     CF_train_data = train_data[:, 50:]
    #
    #     AA_embed_output = self.embed(AA_train_data)
    #     CF_embed_output = self.embed(CF_train_data)
    #     # CF_train_data = CF_train_data.float()
    #     # CF_label = self.full7(CF_train_data)
    #
    #     '''----------------------位置编码------------------------'''
    #     AA_pos_output = self.pos_encoding(self.embed(AA_train_data) * math.sqrt(self.embedding_size))
    #     CF_pos_output = self.pos_encoding(self.embed(CF_train_data) * math.sqrt(self.embedding_size))
    #     '''-----------------------------------------------------'''
    #
    #     '''----------------------attention----------------------'''
    #     AA_attention_output = self.attention_encode(AA_pos_output)
    #     CF_attention_output = self.attention_encode(CF_pos_output)
    #     '''-----------------------------------------------------'''
    #
    #     '''----------------------特征相加-------------------------'''
    #     AA_vectors = AA_embed_output + AA_attention_output
    #     CF_vectors = CF_embed_output + CF_attention_output
    #     '''------------------------------------------------------'''
    #
    #     '''---------------------data_cnn-----------------------'''
    ...
    #
    #     CF_cnn_input = CF_vectors.permute(0, 2, 1)
    #     CF_cnn_output = self.TextCNN(CF_cnn_input)
    #
    #     cnn_output = AA_cnn_output + CF_cnn_output
    ...
    def forward(self, train_data, valid_lens, in_feat=False):
        ...
        '''----------------------位置编码------------------------'''
        pos_output = self.pos_encoding(self.embed(AA_train_data) * math.sqrt(self.embedding_size))
        '''-----------------------------------------------------'''
        ...
        cnn_output0 = torch.cat([AA_cnn_output, CF_cnn_output], dim=-1)
