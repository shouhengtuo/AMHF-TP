...
def evaluate(model, datadl, device):
    ...
    with torch.no_grad():
        ...
        for train_data, train_label, train_length, hyGx_train in datadl:
            train_data = train_data.to(device)
            train_label = train_label.to(device)
            hyGx_train = hyGx_train.to(device)
            predictions.extend(model(train_data, train_length, hyGx_train).tolist())
...
class DataTrain:
    ...
    def train_step(self, train_iter, dataset_test, epochs=None, plot_picture=False):
        ...
        for epoch in range(1, epochs + 1):
            ...
            start = time.time()
            total_loss = 0
            for train_data, train_label, train_length, hyGx_train in train_iter:
                self.model.train()  # 进入训练模式

                train_data, train_label, train_length, hyGx_train= train_data.to(self.device), train_label.to(
                    self.device), train_length.to(self.device), hyGx_train.to(self.device),
                # print(train_label.size())
                # train_label = train_label.view([32, 21])
                # y_hat = self.model(train_data.long(), train_length)
                y_hat = self.model(train_data, train_length, hyGx_train)
                # 训练数据结果
                y_hat.tolist()
            ...
            with torch.no_grad():
                ...
                if test_aiming > best_aiming:
                    ...
                    torch.save(self.model.state_dict(), './result/AM23_12.pkl')
            # 测试数据结果输出
